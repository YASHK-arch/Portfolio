...
const advanceFormation = (state, flow, elapsed, duration, frozen) => {
  ...
  if (state.weights.every((value, i) => Math.abs(value - goal[i]) < 0.0001 && Math.abs(state.velocity[i]) < 0.001)) {
    state.weights = goal; state.velocity.fill(0);
  }
};
...
const advanceHold = (hold, elapsed, disabled) => {
  ...
  const target2 = engaging ? 1 : 0;
  const response = engaging ? 3.8 : 3.2;
  const decay = Math.exp(-response * step);
  const offset = hold.amount - target2;
  const momentum = hold.velocity + response * offset;
  hold.amount = target2 + (offset + momentum * step) * decay;
  hold.velocity = (hold.velocity - response * momentum * step) * decay;
  if (Math.abs(hold.amount - target2) < 0.0001 && Math.abs(hold.velocity) < 0.001) { hold.amount = target2; hold.velocity = 0; }
  if (hold.amount > 0) hold.phase += elapsed * (0.35 + hold.amount * 0.5);
};

const resetPointerMotion = pointer => {
  pointer.velocity ??= [0, 0];
  pointer.velocity[0] = 0; pointer.velocity[1] = 0; pointer.presenceVelocity = 0;
};
...
const advancePointer = (pointer, elapsed) => {
  ...
  if (Math.abs(pointer.presence - presenceTarget) < 0.001 && Math.abs(pointer.presenceVelocity) < 0.01) {
    pointer.presence = presenceTarget; pointer.presenceVelocity = 0;
  }
};
...
const startRipple = (ripples, origin, aspect, strength = 1) => {
  ...
  ripple.origin = [...origin]; ripple.age = 0;
...
const mixColor = (from, to, amount) => [
  ...
  from[2] + (to[2] - from[2]) * amount, 1
];

/* ---- Shader sources (identical to React Bits source) ---- */
const SHARD_SHADER = `
struct ViewParams {
  viewport: vec4f, shape: vec4f, effects: vec4f, composition: vec4f,
  transport: vec4f, formation: vec4f, gather: vec4f, pointer: vec4f,
  shock: vec4f, shockB: vec4f, shockC: vec4f, shockD: vec4f,
  material: vec4f, light: vec4f, environment: vec4f,
...
fn unitFloat(value: u32) -> f32 { return f32(hashU32(value)) * (1.0 / 4294967296.0); }
fn safeNormalize(value: vec3f) -> vec3f { return value / max(length(value), 0.0001); }
fn safeNormalize2(value: vec2f) -> vec2f { return value / max(length(value), 0.0001); }
...
fn sidePath(seedPhase: f32, distance: f32, aspect: f32, mirror: f32) -> PathSample {
  ...
  let derivative = vec3f(mirror*aspect*(cubicDerivative(1.24,1.02,-0.28,0.12,t)+cos(t*pi*4.0+0.34)*pi*4.0*0.055),cubicDerivative(1.38,0.72,-0.56,-1.38,t)+cos(t*pi*2.0-0.6)*pi*2.0*0.04,cos(t*pi*3.0)*pi*3.0*0.18);
  var sample: PathSample; sample.position=vec3f(mirror*aspect*x,y,z); sample.tangent=safeNormalize(derivative); sample.phase=phase; return sample;
}
fn centerPath(seedPhase: f32, distance: f32, aspect: f32) -> PathSample {
  let pi = 3.14159265359;
  let pathLength = 2.3+2.0*aspect+0.35*aspect*aspect;
  let phase = fract(seedPhase+distance/pathLength); let t = centerArc(phase);
  let angle = mix(-0.25*pi,1.75*pi,t); let angleDerivative = 2.0*pi;
  let radius = 0.72+sin(t*pi*4.0)*0.12; let radiusDerivative = cos(t*pi*4.0)*pi*4.0*0.12;
  let derivative = vec3f(aspect*(-sin(angle)*angleDerivative*radius+cos(angle)*radiusDerivative),cos(angle)*angleDerivative*radius+sin(angle)*radiusDerivative,cos(t*pi*2.0)*pi*2.0*0.16);
  var sample: PathSample; sample.position=vec3f(cos(angle)*radius*aspect,sin(angle)*radius,sin(t*pi*2.0)*0.16); sample.tangent=safeNormalize(derivative); sample.phase=phase; return sample;
}
fn fullPath(seedPhase: f32, distance: f32, aspect: f32) -> PathSample {
  let pi = 3.14159265359;
  let pathWidth = 2.44*aspect; let pathLength = sqrt(pathWidth*pathWidth+5.0);
  let phase = fract(seedPhase+distance/pathLength); let t = fullArc(phase);
  let derivative = vec3f(aspect*2.44,cos((t*1.72-0.2)*pi)*1.72*pi*0.54+cos(t*pi*3.0)*pi*3.0*0.12,-sin(t*pi*2.0-0.7)*pi*2.0*0.22);
  var sample: PathSample; sample.position=vec3f(mix(-aspect*1.22,aspect*1.22,t),sin((t*1.72-0.2)*pi)*0.54+sin(t*pi*3.0)*0.12,cos(t*pi*2.0-0.7)*0.22); sample.tangent=safeNormalize(derivative); sample.phase=phase; return sample;
}
fn mobilePath(seedPhase: f32, distance: f32, aspect: f32) -> PathSample {
  let pi = 3.14159265359;
  let pathWidth = 2.56*aspect; let pathLength = sqrt(pathWidth*pathWidth+1.0);
  let phase = fract(seedPhase+distance/pathLength); let t = mobileArc(phase);
  let derivative = vec3f(aspect*2.56,cos(t*pi)*pi*0.28+cos(t*pi*3.0)*pi*3.0*0.06,-sin(t*pi*2.0)*pi*2.0*0.16);
  var sample: PathSample; sample.position=vec3f(mix(-aspect*1.28,aspect*1.28,t),-0.86+sin(t*pi)*0.28+sin(t*pi*3.0)*0.06,cos(t*pi*2.0)*0.16); sample.tangent=safeNormalize(derivative); sample.phase=phase; return sample;
}
fn weightedPath(seedPhase: f32, phaseOffset: f32, aspect: f32, weights: vec4f) -> PathSample {
  let phase = fract(seedPhase+phaseOffset); var result: PathSample; result.position=vec3f(0.0); result.tangent=vec3f(0.0); result.phase=phase;
  if (aspect < 0.82) {
    let compactWeight = weights.x+weights.y+weights.z;
    if (compactWeight > 0.0001) { let compact=mobilePath(phase,0.0,aspect); result.position+=compact.position*compactWeight; result.tangent+=compact.tangent*compactWeight; }
    if (weights.w > 0.0001) { let wide=fullPath(phase,0.0,aspect); result.position+=wide.position*weights.w; result.tangent+=wide.tangent*weights.w; }
  } else {
    if (weights.x>0.0001) { let right=sidePath(phase,0.0,aspect,1.0); result.position+=right.position*weights.x; result.tangent+=right.tangent*weights.x; }
    if (weights.y>0.0001) { let left=sidePath(phase,0.0,aspect,-1.0); result.position+=left.position*weights.y; result.tangent+=left.tangent*weights.y; }
    if (weights.z>0.0001) { let center=centerPath(phase,0.0,aspect); result.position+=center.position*weights.z; result.tangent+=center.tangent*weights.z; }
    if (weights.w>0.0001) { let wide=fullPath(phase,0.0,aspect); result.position+=wide.position*weights.w; result.tangent+=wide.tangent*weights.w; }
  }
  result.tangent=safeNormalize(result.tangent+vec3f(0.0001,0.0,0.0)); return result;
}
fn pointerField(delta: vec2f, radius: f32, flow: vec2f, depth: f32) -> vec2f {
  let offset=delta/max(radius,0.001); let along=dot(offset,flow); let across=dot(offset,vec2f(-flow.y,flow.x));
  let alongSquared=along*along; let layer=depth*inverseSqrt(1.0+depth*depth);
  let bend=(0.22*alongSquared+0.12*layer*along)/(1.0+alongSquared);
  let curvedAcross=(across+bend)/(1.0+layer*0.18);
  return offset*exp(-0.28*alongSquared-1.2*curvedAcross*curvedAcross);
}
fn rippleWave(age: f32) -> f32 {
  if (age<=0.0||age>=${RIPPLE_TAIL}) { return 0.0; }
  return sin(age*10.0)*exp(-age*3.2)*smoothstep(0.0,0.14,age)*(1.0-smoothstep(1.4,${RIPPLE_TAIL},age));
}
fn rippleDisplacement(position: vec3f, pulse: vec4f) -> vec4f {
  ...
  return vec4f(delta/(distance+0.12)*wave*0.28,wave*0.12,abs(wave));
}
fn shardVertex(index: u32) -> vec3f {
  let fold=0.34;
  let vertices=array<vec3f,6>(vec3f(0.0,1.0,fold),vec3f(-0.72,0.0,0.0),vec3f(0.0,-1.0,fold),vec3f(0.0,1.0,fold),vec3f(0.0,-1.0,fold),vec3f(0.72,0.0,0.0));
  return vertices[index%6u];
}
fn softbox(direction: vec3f, center: vec2f, size: vec2f) -> f32 {
  let q=abs((direction.xy-center)/size); let q2=q*q; let q4=q2*q2; return exp(-(q4.x+q4.y));
}
fn aces(color: vec3f) -> vec3f {
  let a=2.51; let b=0.03; let c=2.43; let d=0.59; let e=0.14;
  return clamp((color*(a*color+b))/(color*(c*color+d)+e),vec3f(0.0),vec3f(1.0));
}
@vertex fn vs_main(@builtin(vertex_index) vertexIndex: u32, @builtin(instance_index) instanceIndex: u32) -> VertexOut {
  ...
  let laneWidth=(lane*0.56+flowWave*0.055*view.shape.z)*view.shape.x*widthProfile*(1.0+loose*0.72);
  let depthLane=(seedDepth*2.0-1.0)*view.shape.y+cos(path.phase*31.4159265359+seedLane*8.0)*0.06*view.shape.z;
  ...
  if (view.formation.y+view.formation.z>0.00001) {
    ...
    var formedPosition=renderPosition*view.formation.x; var formedDirection=direction*view.formation.x;
    if (view.formation.y>0.00001) {
      ...
      formedPosition+=position*view.formation.y; formedDirection+=safeNormalize(vec3f(-radial.y,radial.x,radial.x*0.2))*view.formation.y;
    }
    if (view.formation.z>0.00001) {
      let phase=fract(seedPhase+view.viewport.w/(aspect*3.0+2.0));
      let angle=phase*6.28318530718; let ribbonWidth=(seedLane-0.5)*0.54*view.shape.x;
      let position=vec3f(mix(-aspect*1.35,aspect*1.35,phase)+center.x*0.5,sin(angle)*0.42+cos(angle*2.0)*ribbonWidth,(cos(angle)*0.35+sin(angle*2.0)*ribbonWidth+(seedDepth-0.5)*0.12)*view.shape.y);
      let tangent=safeNormalize(vec3f(aspect*2.7,cos(angle)*2.638938,-sin(angle)*2.199115*view.shape.y));
      formedPosition+=position*view.formation.z; formedDirection+=tangent*view.formation.z;
    }
    renderPosition=formedPosition; direction=safeNormalize(formedDirection+vec3f(0.0,0.0,0.02*view.formation.x*(1.0-view.formation.x)));
    planarNormal=safeNormalize2(vec2f(-direction.y,direction.x));
  }
  if (abs(view.pointer.w)>0.0001) {
    let field=pointerField(view.pointer.xy-renderPosition.xy,view.pointer.z,vec2f(planarNormal.y,-planarNormal.x),renderPosition.z);
  ...
  if (view.gather.z>0.00001) {
    ...
    let amount=pow(view.gather.z,1.0+seedDepth*0.65+min(length(relative),4.0)*0.12);
    let curledDirection=safeNormalize(vec3f(-orbit.y,orbit.x,sin(layer)*0.35));
    renderPosition=mix(renderPosition,cluster,amount); direction=safeNormalize(mix(direction,curledDirection,amount));
  }
  var rippleLight=0.0;
  if (view.shock.w+view.shockB.w+view.shockC.w+view.shockD.w>0.0001) {
    let displacement=rippleDisplacement(renderPosition,view.shock)+rippleDisplacement(renderPosition,view.shockB)+rippleDisplacement(renderPosition,view.shockC)+rippleDisplacement(renderPosition,view.shockD);
  ...
  let shapeLocal=shardVertex(vertexIndex); var local=shapeLocal;
  local.x*=mix(0.72,1.08,seedLane); local.y*=mix(0.82,1.12,seedDepth);
  local.x+=(seedDepth-0.5)*(1.0-abs(local.y))*0.16;
  var side=cross(vec3f(0.0,0.0,1.0),direction);
  let sideLengthSquared=dot(side,side);
  if (sideLengthSquared>0.0001) { side*=inverseSqrt(sideLengthSquared); } else { side=vec3f(1.0,0.0,0.0); }
  ...
  let bankedSide=side*cos(roll)+facing*sin(roll);
  let bankedFacing=facing*cos(roll)-side*sin(roll);
  ...
  let world=renderPosition+direction*local.y*size*1.26*view.effects.z+bankedSide*local.x*size*0.72+bankedFacing*local.z*size*0.72;
  ...
  let triangle=vertexIndex/3u; let corner=vertexIndex%3u;
  var mapped=vec3f(0.0); var mappedCrease=vec3f(0.0); var shardAlpha=0.0;
  if (corner==0u) {
    ...
    let broad=softbox(reflection,vec2f(-0.34,0.28)+pointerShift*0.36,vec2f(0.52,0.22)+roughness*0.3);
    let strip=softbox(reflection,vec2f(0.48,-0.08)-pointerShift*0.2,vec2f(0.12,0.72));
    let diffuse=max(dot(normal,keyDirection),0.0);
    let specular=pow(max(dot(normal,halfDirection),0.0),mix(92.0,9.0,roughness));
    let fresnelBase=1.0-max(dot(normal,viewDirection),0.0);
    let fresnel=fresnelBase*fresnelBase*fresnelBase*fresnelBase;
    ...
    color+=view.highlightColor.rgb*(broad*mix(0.3,0.86,1.0-roughness))*(1.0+view.material.w*0.14);
    color+=view.accentColor.rgb*strip*(0.12+fresnel*0.42);
    color+=view.highlightColor.rgb*specular*mix(0.82,1.0,seedDepth);
    color+=mix(view.baseColor.rgb,view.accentColor.rgb,seedLane)*fresnel*(0.15+view.material.w*0.16);
    color+=view.accentColor.rgb*(broad*0.045+fresnel*0.075)*view.material.w;
    var creaseColor=color+view.highlightColor.rgb*(0.08+specular*0.22);
    let fill=mix(view.accentColor.rgb,view.baseColor.rgb,depthFog)*(0.38+diffuse*0.12)*facet*view.environment.x;
    color+=fill; creaseColor+=fill;
    let pulseColor=mix(view.accentColor.rgb,view.highlightColor.rgb,0.18);
    color+=pulseColor*rippleLight*(0.85+fresnel*0.45); creaseColor+=pulseColor*rippleLight*1.35;
    let fog=mix(0.42,1.0,depthFog);
    let exposure=fog*view.material.z*view.effects.w;
    mapped=aces(color*exposure); mappedCrease=aces(creaseColor*exposure);
  ...
  var out: VertexOut;
  out.position=vec4f(ndc,depth,1.0); out.baseAlpha=vec4f(mapped,shardAlpha);
  out.creaseColor=mappedCrease-mapped; out.localCoord=shapeLocal.xy;
  return out;
}
@fragment fn fs_main(in: VertexOut) -> @location(0) vec4f {
  let crease=(1.0-smoothstep(0.015,0.11,abs(in.localCoord.x)))*(1.0-smoothstep(0.78,1.0,abs(in.localCoord.y)));
  var coverage=1.0;
...
fn visibleResidual(uv: vec2f) -> vec4f {
  ...
  let contribution=smoothstep(post.bloomInfo.z-post.bloomInfo.w,post.bloomInfo.z+post.bloomInfo.w,energy)*energy;
  return vec4f(mix(residual*smoothstep(post.bloomInfo.z-post.bloomInfo.w,post.bloomInfo.z+post.bloomInfo.w,energy),post.tint.rgb*contribution,post.finishing.w),contribution);
}
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let offset=post.bloomInfo.xy*0.25;
  var glow=visibleResidual(uv+offset)+visibleResidual(uv-offset)+visibleResidual(uv+vec2f(offset.x,-offset.y))+visibleResidual(uv+vec2f(-offset.x,offset.y));
  return glow*0.25;
}
`;
...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let nearOffset=blur.direction.xy*1.3846153846; let farOffset=blur.direction.xy*3.2307692308;
  return textureSampleLevel(bloomTexture,linearSampler,uv,0.0)*0.2270270270
    +textureSampleLevel(bloomTexture,linearSampler,uv+nearOffset,0.0)*0.3162162162
    +textureSampleLevel(bloomTexture,linearSampler,uv-nearOffset,0.0)*0.3162162162
    +textureSampleLevel(bloomTexture,linearSampler,uv+farOffset,0.0)*0.0702702703
    +textureSampleLevel(bloomTexture,linearSampler,uv-farOffset,0.0)*0.0702702703;
}
`;
...
fn hash12(value: vec2f) -> f32 {
  let p=fract(value*vec2f(0.1031,0.1030)); let mixed=p+dot(p,p.yx+33.33);
  return fract((mixed.x+mixed.y)*mixed.x);
}
@fragment fn fs_main(@location(0) uv: vec2f,@builtin(position) pixel: vec4f) -> @location(0) vec4f {
  ...
  if (post.finishing.y>0.0001) {
    ...
    foreground+=vec3f(noise*post.finishing.y*smoothstep(0.008,0.18,length(foreground)));
  }
...
const createRenderGraph = (gpu, outputSize, bloomSize = resolveBloomSize(outputSize)) => {
  const viewParams = uniforms(gpu, {
    viewport: [1, 0.0132, 1, 0], shape: [1, 1, 0.36, 0], effects: [1, 2, 1, 1.12],
    composition: [0, 0, 0, 1], transport: [0, 0, 0, 0], formation: [1, 0, 0, 0],
    gather: [0, 0, 0, 0], pointer: [0, 0, 0.54, 0],
    shock: [0, 0, 4, 0], shockB: [0, 0, 0, 0], shockC: [0, 0, 0, 0], shockD: [0, 0, 0, 0],
    material: [0.46, MATERIALS.pearl, 0.92, 0.54], light: [-0.321, 0.49, 0.845, 0],
    environment: [0, 0, 0, 0],
  ...
  const postParams = uniforms(gpu, {
    ...
    finishing: [0.5, 0.05, 0.0075, 0], background: [0.071, 0.059, 0.09, 1],
    temporal: [0, 0, 0, 0], tint: [137 / 255, 106 / 255, 189 / 255, 1]
  });
  ...
  const sceneTarget = target(gpu, { size: outputSize, format: 'rgba8unorm', label: 'aero-shards-scene' });
  ...
  const bloomEffect = effect(gpu, BLOOM_SHADER, { label: 'aero-shards-bloom-prefilter', set: { sceneTexture: sceneTarget, sceneSampler: linearSampler, post: postParams } });
  const blurParamsX = uniforms(gpu, { direction: [1 / bloomSize[0], 0, 0, 0] });
  const blurParamsY = uniforms(gpu, { direction: [0, 1 / bloomSize[1], 0, 0] });
  const bloomBlurX = effect(gpu, BLOOM_BLUR_SHADER, { label: 'aero-shards-bloom-horizontal', set: { bloomTexture: bloomTarget, linearSampler, blur: blurParamsX } });
  const bloomBlurY = effect(gpu, BLOOM_BLUR_SHADER, { label: 'aero-shards-bloom-vertical', set: { bloomTexture: bloomScratchTarget, linearSampler, blur: blurParamsY } });
  const finishEffect = effect(gpu, FINISH_SHADER, { label: 'aero-shards-finish', set: { sceneTexture: sceneTarget, bloomTexture: bloomTarget, linearSampler, post: postParams } });
  return { viewParams, postParams, shardDraw, sceneTarget, bloomTarget, bloomScratchTarget, bloomEffect, blurParamsX, blurParamsY, bloomBlurX, bloomBlurY, finishEffect };
};
...
export default function AeroShards({
  ...
  effect: effectProp = 'none', scale = 1, spread = 1, depth = 1, speed = 1, spin = 1,
  ...
  rippleIntensity = 1, holdToGather = true, paused = false, className = '', onError
}) {
  ...
  const resolvedEffect = EFFECTS[effectProp] ?? EFFECTS.none;
  ...
  const backgroundLuma = resolvedBackground[0] * 0.2126 + resolvedBackground[1] * 0.7152 + resolvedBackground[2] * 0.0722;
  ...
  settingsRef.current = {
    ...
    accent: resolvedAccentColor, composition: PLACEMENTS[placement] ?? PLACEMENTS.full,
    flow: FLOWS[flow] ?? FLOWS.stream, material: MATERIALS[material] ?? MATERIALS.pearl,
    effect: resolvedEffect, detailCount: resolvedDetail.count * resolvedDensity * effectDetail,
    ...
    spread: resolvedSpread, depth: resolvedDepth, roughness: resolvedMaterial.roughness,
    brightness: resolvedMaterial.brightness, glow: resolvedMaterial.glow * resolvedGlow,
    edgeSoftness: resolvedEdgeSoftness, bloom: resolvedBloom, grain: resolvedGrain,
    chromaticAberration: resolvedChromaticAberration, exposure: 1.12 + (0.96 - 1.12) * lightSurface,
    lightSurface, transitionDuration: resolvedTransitionDuration,
    interaction: resolvedInteraction, interactionRadius: (interaction === 'attract' ? 0.27 : 0.18) * resolvedInteractionRadius,
    interactionStrength: resolvedInteractionStrength, rippleIntensity: clamp(rippleIntensity, 0, 2),
    holdToGather, paused,
    signature: [backgroundColor, shardColor, accentColor, placement, flow, material, detail, effectProp, resolvedScale, resolvedSpread, resolvedDepth, resolvedSpeed, resolvedSpin, interaction, resolvedDensity, resolvedShardSize, resolvedStretch, resolvedTurbulence, resolvedGlow, resolvedEdgeSoftness, resolvedBloom, resolvedGrain, resolvedChromaticAberration, resolvedTransitionDuration, resolvedInteractionRadius, resolvedInteractionStrength, rippleIntensity, holdToGather, paused].join('|')
  };
  ...
  useEffect(() => {
    ...
    let disposed = false, runtimeFailed = false, gpu;
    let animationFrameId = 0, timeoutId = 0;
    let unsubscribeResize, unsubscribeGpuError, visibilityObserver, resizeObserver;
    ...
    const reportFailure = error => {
      ...
      unsubscribeResize?.(); unsubscribeGpuError?.();
      const failedGpu = gpu; gpu = undefined; failedGpu?.dispose();
    ...
    const updatePointerTarget = next => {
      ...
      if (!pointer.initialized || (!pointer.active && pointer.presence === 0)) {
        pointer.raw = [...next]; pointer.position = [...next]; pointer.presence = 0;
        resetPointerMotion(pointer); pointer.initialized = true;
      } else { pointer.raw[0] = next[0]; pointer.raw[1] = next[1]; }
      pointer.active = 1;
    };
    const deactivatePointer = () => {
      pointerRef.current.active = 0; holdRef.current.pointerId = null;
      const now = performance.now(); interactionDeadline = now + 140; settlingDeadline = now + 680;
      wakeRenderer();
    };
    const handlePointerMove = event => {
      if (!event.isPrimary || !visible || settingsRef.current.interaction === INTERACTIONS.none) return;
      const next = pointFromClient(event.clientX, event.clientY);
      if (!next) { if (pointerRef.current.active || pointerRef.current.presence > 0) deactivatePointer(); return; }
      updatePointerTarget(next);
      const now = performance.now(); interactionDeadline = now + 140; settlingDeadline = now + 680;
      wakeRenderer();
    };
    const handlePointerDown = event => {
      if (!event.isPrimary || event.button !== 0 || !visible || settingsRef.current.interaction === INTERACTIONS.none) return;
      if (event.target instanceof Element && event.target.closest('a,button,input,textarea,select,[role="button"],[contenteditable="true"]')) return;
      const next = pointFromClient(event.clientX, event.clientY);
      if (!next) return;
      ...
      if (!settings.paused && !reduceMotion.matches && settings.speed > 0.0001) {
        ...
        if (settings.holdToGather) { holdRef.current.pointerId = event.pointerId; holdRef.current.elapsed = 0; }
      }
      updatePointerTarget(next);
      const now = performance.now(); interactionDeadline = now + 220; settlingDeadline = now + 800;
      wakeRenderer();
    };
    const handlePointerEnd = event => {
      ...
      if (hold.pointerId === event.pointerId) {
        hold.pointerId = null;
        const settings = settingsRef.current;
        if (hold.amount > 0.1 && !settings.paused && !reduceMotion.matches && settings.interaction !== INTERACTIONS.none)
          startRipple(ripplesRef.current, pointerRef.current.raw, bounds.width / Math.max(bounds.height, 1), 1 + hold.amount * 0.8);
        wakeRenderer();
      }
    ...
    window.addEventListener('scroll', () => { boundsDirty = true; }, { passive: true, capture: true });
    document.addEventListener('visibilitychange', () => { resumePending = true; holdRef.current.pointerId = null; resetPointerMotion(pointerRef.current); wakeRenderer(); });
    reduceMotion.addEventListener('change', () => { resumePending = true; wakeRenderer(); });

    visibilityObserver = new IntersectionObserver(entries => {
      const entry = entries[0];
      visibilityRatio = entry?.intersectionRatio ?? 1;
      visible = entry ? entry.isIntersecting && visibilityRatio >= 0.02 : true;
      if (!visible) { interactionDeadline = 0; settlingDeadline = 0; pointerRef.current.active = 0; pointerRef.current.presence = 0; holdRef.current.pointerId = null; resetPointerMotion(pointerRef.current); }
      else resumePending = true;
      wakeRenderer();
    ...
    void (async () => {
      try {
        ...
        let lastSettingsSignature = '', previousRenderTimestamp = 0, lastPresentationTimestamp = 0;
        let nextPresentationTimestamp = 0, flowDistance = 0, travelPhase = 0, grainTime = 0, firstFrame = true;
        ...
        let renderScale = settingsRef.current.scale, runtimeQualityLevel = 0, appliedBloomLevel = 0;
        let pendingBloomResize = false, pressureStartedAt = 0, stableStartedAt = performance.now();
        let lastQualityChange = 0, encodeAverage = 0, renderTimestamp = 0, previousRafTimestamp = 0;
        let refreshInterval = 1000 / 60;
        const refreshSamples = new Float32Array(30); let refreshSampleCount = 0, refreshSampleIndex = 0;

        const resolveFrameState = now => {
          const pointer = pointerRef.current;
          if (now < interactionDeadline || Math.abs(pointer.presence - pointer.active) > 0.004 ||
              ripplesRef.current.some(r => r.strength > 0) || layoutTransitioning ||
              holdRef.current.pointerId !== null || holdRef.current.amount > 0 ||
              formation.weights[settingsRef.current.flow] < 1 ||
              Math.abs(settingsRef.current.scale - renderScale) > 0.001) return FRAME_STATES.interactive;
        ...
        const resizePostTargets = (qualityLevel = appliedBloomLevel) => {
          const [w, h] = [Math.max(1, output.size[0]), Math.max(1, output.size[1])];
          const bloomSize = resolveBloomSize([canvas.clientWidth, canvas.clientHeight], qualityLevel);
          graph.sceneTarget.resize([w, h]); graph.bloomTarget.resize(bloomSize); graph.bloomScratchTarget.resize(bloomSize);
          graph.blurParamsX.set({ direction: [1 / bloomSize[0], 0, 0, 0] });
          graph.blurParamsY.set({ direction: [0, 1 / bloomSize[1], 0, 0] });
          graph.postParams.set({ viewport: [w, h, 1 / w, 1 / h], bloomInfo: [1 / bloomSize[0], 1 / bloomSize[1], 0.2, 0.12] });
        };

        unsubscribeResize = output.onResize(() => { resizePostTargets(); needsRender = true; wakeRenderer(); });
        resizeObserver = new ResizeObserver(() => {
          updateBounds(); const dpr = resolveDpr(preset, canvas);
          const nextSize = [Math.max(1, Math.round(canvas.clientWidth * dpr)), Math.max(1, Math.round(canvas.clientHeight * dpr))];
          if (nextSize[0] !== output.size[0] || nextSize[1] !== output.size[1]) output.resize(nextSize);
          resizePostTargets(); wakeRenderer();
        });
        resizeObserver.observe(canvas);

        const setRuntimeQuality = (nextLevel, now, frameState) => {
          const cl = Math.max(0, Math.min(RUNTIME_QUALITY.length - 1, nextLevel));
          if (cl === runtimeQualityLevel) return;
          runtimeQualityLevel = cl; pressureStartedAt = 0; stableStartedAt = now; lastQualityChange = now;
          if (frameState === FRAME_STATES.interactive || frameState === FRAME_STATES.settling) pendingBloomResize = true;
          else { appliedBloomLevel = runtimeQualityLevel; pendingBloomResize = false; resizePostTargets(); }
        };

        const renderFrame = currentFrame => {
          const settings = settingsRef.current;
          const frozen = settings.paused || reduceMotion.matches || settings.speed <= 0.0001;
          const elapsed = resumePending || !previousRenderTimestamp ? 0 : Math.min(0.05, Math.max(0, (renderTimestamp - previousRenderTimestamp) / 1000));
          resumePending = false; previousRenderTimestamp = renderTimestamp; lastSettingsSignature = settings.signature; needsRender = false;
          ...
          layoutWeights = placementMotion.weights; layoutTransitioning = layoutWeights[settings.composition] !== 1;
          ...
          advanceHold(holdRef.current, elapsed, frozen || !settings.holdToGather || settings.interaction === INTERACTIONS.none);
          if (settings.interaction === INTERACTIONS.none) { pointer.active = 0; pointer.presence = 0; resetPointerMotion(pointer); }
          else if (frozen) { pointer.position = [...pointer.raw]; pointer.presence = pointer.active; resetPointerMotion(pointer); }
          else if (pointer.initialized) advancePointer(pointer, elapsed);
          advanceRipples(ripplesRef.current, elapsed, frozen || settings.interaction === INTERACTIONS.none);
          const rq = RUNTIME_QUALITY[runtimeQualityLevel];
          const activeCount = Math.max(700, Math.round(preset.count * settings.detailCount * rq.countScale));
          const dc = Math.pow(1 / rq.countScale, 0.2);
          ...
          const psx = (pointer.position[0] - 0.5) * 0.38 * lightPresence;
          const psy = (pointer.position[1] - 0.5) * -0.24 * lightPresence;
          const lx = -0.38 + psx, ly = 0.58 + psy, ll = Math.hypot(lx, ly, 1);
          const isign = settings.interaction === INTERACTIONS.attract ? 1 : -1;
          const ip = settings.interaction === INTERACTIONS.none ? 0 : pointer.presence * settings.interactionStrength * isign * (1 - holdRef.current.amount);
          const inv = 1 / renderScale;
          const pwx = ((pointer.position[0] * 2 - 1) * aspect) / renderScale;
          const pwy = (1 - pointer.position[1] * 2) / renderScale;
          const ru = ripplesRef.current.map(r => [(r.origin[0] * 2 - 1) * aspect * inv, (1 - r.origin[1] * 2) * inv, r.age, r.strength * settings.interactionStrength * settings.rippleIntensity * inv]);
          graph.viewParams.set({
            viewport: [aspect, 0.0125 * settings.shardSize * dc, renderScale, flowDistance],
            shape: [settings.spread, settings.depth, settings.turbulence, psy],
            effects: [settings.spin, settings.edgeSoftness, settings.stretch, settings.exposure],
            composition: layoutWeights, transport: [travelPhase, Math.min(1, (1 - Math.max(...layoutWeights)) * 12), 0, 0],
            formation: formation.weights, gather: [pwx, pwy, holdRef.current.amount, holdRef.current.phase],
            pointer: [pwx, pwy, settings.interactionRadius * 2 * inv, ip * inv],
            shock: ru[0], shockB: ru[1], shockC: ru[2], shockD: ru[3],
            material: [settings.roughness, settings.material, settings.brightness, settings.glow],
            light: [lx / ll, ly / ll, 1 / ll, psx], environment: [settings.lightSurface, 0, 0, 0],
            baseColor: settings.shard, highlightColor: settings.highlight, accentColor: settings.accent
          });
          graph.postParams.set({
            finishing: [settings.bloom, settings.grain, settings.chromaticAberration, settings.lightSurface],
            background: settings.background, tint: mixColor(settings.shard, settings.accent, 0.4),
            temporal: [grainTime, 0, 0, 0]
          });
          const postEnabled = settings.bloom > 0.0001 || settings.grain > 0.0001 || settings.chromaticAberration > 0.000001;
          if (!postEnabled) {
            currentFrame.pass({ target: output, clear: settings.background }, pass => { pass.draw(graph.shardDraw, { instances: activeCount }); });
          } else {
            currentFrame.pass({ target: graph.sceneTarget, clear: settings.background }, pass => { pass.draw(graph.shardDraw, { instances: activeCount }); });
            if (settings.bloom > 0.0001) {
              currentFrame.pass({ target: graph.bloomTarget, clear: [0, 0, 0, 1] }, pass => { pass.draw(graph.bloomEffect); });
              currentFrame.pass({ target: graph.bloomScratchTarget, clear: [0, 0, 0, 1] }, pass => { pass.draw(graph.bloomBlurX); });
              currentFrame.pass({ target: graph.bloomTarget, clear: [0, 0, 0, 1] }, pass => { pass.draw(graph.bloomBlurY); });
            }
            currentFrame.pass({ target: output, clear: settings.background }, pass => { pass.draw(graph.finishEffect); });
          }
        ...
        const scheduleSleep = ts => { if (disposed || runtimeFailed || timeoutId || animationFrameId || !visible || document.hidden) return; timeoutId = window.setTimeout(() => { timeoutId = 0; scheduleRaf(); }, Math.max(0, ts - performance.now() - 10)); };
        const scheduleFrame = timestamp => {
          animationFrameId = 0;
          if (disposed || runtimeFailed || !visible || document.hidden) return;
          if (previousRafTimestamp) {
            const s = timestamp - previousRafTimestamp;
            if (s > 3 && s < 35) { refreshSamples[refreshSampleIndex] = s; refreshSampleIndex = (refreshSampleIndex + 1) % refreshSamples.length; refreshSampleCount = Math.min(refreshSampleCount + 1, refreshSamples.length); refreshInterval = Math.min(...Array.from(refreshSamples.slice(0, refreshSampleCount))); }
          }
          previousRafTimestamp = timestamp;
          const settings = settingsRef.current;
          const frozen = settings.paused || reduceMotion.matches || settings.speed <= 0.0001;
          if (frozen && !firstFrame && settings.signature === lastSettingsSignature && !needsRender) return;
          const forceFrame = firstFrame || settings.signature !== lastSettingsSignature || !lastPresentationTimestamp;
          const frameState = resolveFrameState(performance.now());
          const pi = resolveFrameInterval(frameState, refreshInterval);
          const cadenceDeadline = lastPresentationTimestamp + pi;
          const dueTimestamp = nextPresentationTimestamp ? Math.min(nextPresentationTimestamp, cadenceDeadline) : cadenceDeadline;
          if (!forceFrame && timestamp < dueTimestamp - 0.5) { if (frameState.continuous) scheduleRaf(); else scheduleSleep(dueTimestamp); return; }
          ...
          try { frame(gpu, renderFrame); } catch (e) { reportFailure(e); return; }
          ...
          const missedDeadline = Number.isFinite(sincePresentation) && sincePresentation > pi * 1.65;
          const underPressure = encodeAverage > 4 || missedDeadline;
          if (underPressure) { if (!pressureStartedAt) pressureStartedAt = monitorNow; } else pressureStartedAt = 0;
          if (underPressure || frameState !== FRAME_STATES.ambient) stableStartedAt = monitorNow;
          if (runtimeQualityLevel < RUNTIME_QUALITY.length - 1 && monitorNow - lastQualityChange > 2200 && pressureStartedAt > 0 && monitorNow - pressureStartedAt > 1800) setRuntimeQuality(runtimeQualityLevel + 1, monitorNow, frameState);
          else if (runtimeQualityLevel > 0 && frameState === FRAME_STATES.ambient && !underPressure && monitorNow - stableStartedAt > 15000 && monitorNow - lastQualityChange > 15000) setRuntimeQuality(runtimeQualityLevel - 1, monitorNow, frameState);
          const nextState = resolveFrameState(performance.now());
          nextPresentationTimestamp = advanceFrameDeadline(timestamp, dueTimestamp, resolveFrameInterval(nextState, refreshInterval), forceFrame || nextState.continuous !== frameState.continuous);
          if (frozen) return;
          if (nextState.continuous) scheduleRaf(); else scheduleSleep(nextPresentationTimestamp);
        };
